...
This article aims to replicate the case study developed by [Araujo et al. (in review)](https://github.com/AraujoMat/treesliceR). Moreover, here we will present the workflow executed within the study for both a single passerine phylogeny and multiple passerine phylogenies, in order to control the outputted results for phylogenetic uncertainty [Rangel et al., 2015](https://doi.org/10.1111/evo.12644).

Before starting our analysis, we'll need to load the `ape`, `ggplot2`, and `ggpubr` packages:
```{r, eval = FALSE}
...
Additionally, we'll need to load (and install if necessary) the `treesliceR` package:
```{r}
